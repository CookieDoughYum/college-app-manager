...
- [x] `/activities` renders interests questionnaire with clickable tag chips
- [x] `/activities` renders 4-year course plan grid with 4 columns; 11th grade column shows warning style
- [x] `/exams` renders SAT/ACT section, AP tracker (chips addable via state), and study schedule placeholder
- [x] `/colleges` renders major questionnaire, recommended majors chips, and college list with reach/target/safety badges
- [x] All three screens are reachable from the sidebar
- [x] No API calls are made — all state is local `useState`
- [x] Form inputs accept user input (controlled components)
